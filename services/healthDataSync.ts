...
class HealthDataSyncService {
  ...
  async getLastSyncTime(): Promise<number> {
    try {
      ...
      handleError(error, 'healthDataSync.getLastSyncTime');
  ...
  async syncFromHealthPlatform(config: SyncConfig = {}): Promise<SyncResult> {
    ...
    try {
      ...
      const now = new Date();
      const startCal = localCalendarDayDateFromDayKeyMs(
        localDayKeyPlusCalendarDaysFromNow(-lookbackDays)
      );
      const timeRange = {
        startTime: localDayStartMs(startCal),
        endTime: localDayClosedRangeMaxMs(now),
      };
