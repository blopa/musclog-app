...
- **Fasting Days (opt-in)**: Gated behind `ENABLE_FASTED_DAY_SETTING_TYPE` (`enable_fasted_day`, default off; Advanced settings → Nutrition). Lets the user flag a day with **no logged food** as an intentional fast, persisted in the `fasted_days` table (one soft-deletable row per day, `date` + `timezone` stamped like `nutrition_logs` so `utcNormalizedDayKey(date, timezone)` shares the exact bucket space). All access goes through `FastedDayRepository` (`markFasted`/`unmarkFasted`/`isFasted`/`getFastedDayKeys`/`getAllFastedDayKeys`). **Unified denominator rule** (only applied when the setting is on): every historical macro/calorie **average** divides the period total by `effectiveNutritionDayCount` (`utils/effectiveNutritionDays.ts`) = **distinct days with logged food ∪ distinct flagged fasted days**. Unflagged empty days (a forgotten log) are excluded; a flagged fasted day is a real 0-kcal day included in the denominator. This makes the previously-inconsistent divisors converge across `NutritionService.getRangeNutrients` (home weekly card + AI range), `getHistoricalNutritionParams` / empirical TDEE, `ProgressService.calculateInsights` (fasted days are injected into `nutritionDaily` as 0-kcal entries, so averages/weekly/correlation/mood/empirical-TDEE all count them), and `NutritionCheckinService.getCheckinMetrics`. Fasted days also count as logged days for `getMacroLoggingStreak`/`getNutritionStreak` (streak preserved); mark/unmark busts the `MACRO_STREAK_STATE` cache. UI lives in the food diary (`app/app/nutrition/food.tsx` + `components/nutrition/FastingDayCard.tsx`): the mark/unmark affordance appears only on empty, non-future days when the feature is on, confirmed via `ConfirmationModal`. When the setting is **off**, every calc site keeps its exact legacy behavior and no `fasted_days` rows are consulted.
