...
- **Camera (two stacks, deliberately)**: the smart camera (barcode / AI food scanning) uses `react-native-vision-camera` behind the `components/CameraView.tsx` wrapper; `expo-camera` remains **only** for the reps-recording test screen (`app/app/test/reps-recording.tsx`). Their CameraX versions conflict — vision-camera 4.7.3 links against CameraX 1.5.0-alpha03 internals (`Camera2CameraInfoImpl`) that expo-camera's CameraX 1.6.0 removed — so `plugins/withCameraXVersionPin.js` forces every `androidx.camera` artifact to 1.5.0-alpha03. Without the pin the Android app crashes at boot with `NoClassDefFoundError`. Remove the pin (and its block in `android/build.gradle`) when vision-camera is upgraded to v5+ (CameraX 1.7+) or expo-camera is dropped.
