...
- **First gallery pick after boot stalls ~25s — root cause is Expo's single-threaded `modulesQueue`, NOT a per-call cold start**: on the **first** gallery pick within ~25s of a cold boot, tapping "load from gallery" showed a ~25s spinner before the picker/crop UI appeared (observed Pixel 10 / Android 17); the second pick — and any pick after idling ~25s post-boot with no interaction — is instant. Earlier notes here misattributed this to a one-time-per-process native init inside `openCropperAsync` and the system photo picker; that was **wrong** and the user's repro disproves it: a lazy per-call init cannot be cleared by merely _waiting_ 25s doing nothing. **Confirmed cause**: every Expo `AsyncFunction`/`Coroutine` that uses the default queue runs on ONE shared background thread — `expo.modules.AsyncFunctionQueue`, a single `HandlerThread` (`AppContext.modulesQueue`; see expo-modules-core `AppContext.kt`). During boot that one thread is saturated/starved by other modules' async work, so the crop tool's `openCropperAsync` **and** expo-image-picker's `launchImageLibraryAsync`/`requestMediaLibraryPermissionsAsync` (all default-queue Expo functions) queue behind it for tens of seconds. That's why `legacy: true` (below) didn't help — the picker _type_ was never the cause — and why both the crop and the picker showed the same ~22–25s stall. Corroborated by [expo/expo#34531](https://github.com/expo/expo/issues/34531) (heavy default-queue async work blocks unrelated modules). **Fixes applied**: (1) `patches/@bsky.app+expo-image-crop-tool+0.5.1.patch` moves `openCropperAsync` to `Queues.MAIN` (it only does `startActivityForResult`, a Main-thread op anyway, so it no longer waits on the saturated shared thread); (2) `pickFromGallery` skips `requestMediaLibraryPermissionsAsync` on Android (the `ACTION_GET_CONTENT` picker grants per-item access, so it's unneeded — one fewer default-queue call); (3) `pickFromGallery` logs `gallery permission`/`gallery picker` phase timings (always-on, readable via `adb logcat` on a release build) so the residual boot-time offender that saturates `modulesQueue` can be pinpointed. Note: expo-image-picker's `launchImageLibraryAsync` is a `Coroutine` on the default queue and was **not** patched (its infix DSL makes `.runOnQueue` fragile, and its post-pick `readExtras`/`copyFile` must stay off the Main thread); if telemetry shows the picker launch itself (not the crop) is still the stall, either move it to `Dispatchers.IO` via a patch or reduce the boot-time default-queue work.
- **Gallery pick uses the modern system Photo Picker**: `pickFromGallery` in `hooks/useCameraCaptureFlow.ts` calls `ImagePicker.launchImageLibraryAsync` with no `legacy` flag, so on Android it opens expo-image-picker's default `ACTION_PICK_IMAGES`-backed system Photo Picker (backed by `androidx.activity`'s `PickVisualMedia` contract, which also needs no media-library permission — see the Android permission-skip above). `legacy: true` (the old `ACTION_GET_CONTENT` document UI) was removed once the ~25s stall was root-caused to the shared-`modulesQueue` contention documented above — the picker component itself was never the cause, so there was no reason left to keep the older, less capable UI. **`launchImageLibraryAsync` itself is still an unpatched default-queue `Coroutine`** (see the note above): if `adb logcat`'s `gallery picker` phase timing ever shows a large value again, that's this call queuing behind boot-time `modulesQueue` work, not a picker-UI regression.
