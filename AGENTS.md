...
- **Crop tool is gallery-only**: photos captured with the in-app camera (shutter) go **straight to barcode/AI processing with no crop step**; the native crop tool (`@bsky.app/expo-image-crop-tool`) runs **only** when the user picks an existing photo from the gallery (see `hooks/useCameraCaptureFlow.ts` — `takePicture` skips crop, `pickFromGallery` keeps it). The reason is UX: the shutter image was already framed in the live preview, while an existing photo usually needs reframing. `patches/@bsky.app+expo-image-crop-tool+0.5.1.patch` also moves `openCropperAsync` to `Queues.MAIN` (it only does `startActivityForResult`, a Main-thread op anyway, so it never waits behind the shared background queue described below); keep that patch as cheap defense-in-depth even though the SecureStore fix removes the underlying contention.
...
- **Gallery pick uses the modern system Photo Picker, no permission prompt**: `pickFromGallery` in `hooks/useCameraCaptureFlow.ts` calls `ImagePicker.launchImageLibraryAsync` with no `legacy` flag, so it opens the platform picker (Android `ACTION_PICK_IMAGES` via `androidx.activity`'s `PickVisualMedia`; iOS `PHPicker`) — both return only the user-picked item through a temporary content grant and need **no** media-library permission, so the preceding `requestMediaLibraryPermissionsAsync` call was dropped. `legacy: true` (the old `ACTION_GET_CONTENT` document UI) was a failed workaround for the stall above (the picker component was never the cause) and was removed here and from the three image-picker modals (`CreateMealModal`, `CreateCustomFoodModal`, `CreateExerciseModal`). `pickFromGallery` still logs the `gallery picker` phase timing (`adb logcat`, release build) so a residual queue stall would still surface.
