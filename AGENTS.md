...
- **Crop tool is gallery-only**: photos captured with the in-app camera (shutter) go **straight to barcode/AI processing with no crop step**; the native crop tool (`@bsky.app/expo-image-crop-tool`) runs **only** when the user picks an existing photo from the gallery (see `hooks/useCameraCaptureFlow.ts` — `takePicture` skips crop, `pickFromGallery` keeps it). This is deliberate: the **first** `openCropperAsync` call in a process has a ~22–24s stall on some fresh installs (observed on Pixel 10 / Android 17) that happens _before_ the crop activity even launches, while the app is silent. Root cause is still **unconfirmed** — it is NOT the Expo Record/reflection conversion (a patch swapping Record→Map made zero difference and was removed) and NOT missing AOT (`cmd package compile -m speed` didn't help); it's silent one-time-per-process native work inside the first `openCropperAsync` (the second crop in the same process is ~10ms). Keeping crop off the camera path avoids the stall on the common flow; the occasional gallery crop can still pay it once per process.
- **Gallery pick forces the legacy Android picker**: `pickFromGallery` in `hooks/useCameraCaptureFlow.ts` passes `legacy: true` to `ImagePicker.launchImageLibraryAsync`. This is a **separate** stall from the crop-tool one above: expo-image-picker 57 defaults to the modern system Photo Picker (`ACTION_PICK_IMAGES`), whose **cold first-launch-per-process** stalled ~25s _before its grid appeared_ on a Pixel 10 / Android 17 (reproduced 2026-07 — before the grid, first pick per session only, Google Photos backup on but fully synced). The picker runs in a separate, Play-System-updatable MediaProvider/photopicker process and does a latency-sensitive cloud-media handshake on open; the stall is in that system component, **not our code** (expo hands off almost instantly). `legacy` routes to the old `ACTION_GET_CONTENT` document UI, which opens immediately. There's no app-side way to pre-warm the system picker, so `legacy` is the only lever. Trade-off: older document-style UI that can also pull from Drive/Files; iOS ignores the flag. Remove/retest once a newer photo-picker module fixes the cold start.
