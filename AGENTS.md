...
- **Gallery pick forces the legacy Android picker**: `pickFromGallery` in `hooks/useCameraCaptureFlow.ts` passes `legacy: true` to `ImagePicker.launchImageLibraryAsync`. This routes to the old `ACTION_GET_CONTENT` document UI instead of expo-image-picker 57's default modern system Photo Picker (`ACTION_PICK_IMAGES`). It was originally added to dodge a suspected photo-picker cold-start stall, but that stall is now understood to be the shared-`modulesQueue` contention documented above (not the picker component), so `legacy` is **not** the fix for the ~25s stall. It's retained only for its UI trade-off (document-style picker that can also pull from Drive/Files; iOS ignores the flag). Re-evaluate keeping it once the `modulesQueue` contention is fully resolved.
