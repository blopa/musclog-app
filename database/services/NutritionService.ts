...
export class NutritionService {
   ...
   */
  static async getNutritionLogsForMeal(
    date: Date,
    mealType: MealType
  ): Promise<NutritionLog[]> {
    const dateTimestamp = new Date(date.getFullYear(), date.getMonth(), date.getDate()).getTime();
  ...
  static async getDailyNutrients(date: Date): Promise<{
    ...
    carbs: number;
    fat: number;
    byMealType: Record<MealType, { calories: number; protein: number; carbs: number; fat: number }>;
  }> {
  ...
  static async getRangeNutrients(
    ...
    carbs: number;
    fat: number;
    dailyAverages: { calories: number; protein: number; carbs: number; fat: number };
  }> {
  ...
  static async getNutritionStreak(): Promise<number> {
    ...
    for (const dateTimestamp of uniqueDates) {
      ...
      const daysDiff = Math.floor((expectedDate.getTime() - currentDate.getTime()) / (24 * 60 * 60 * 1000));
