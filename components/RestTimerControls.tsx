...
export function RestTimerControls({ onMinus5s, onSkipRest, onPlus5s }: RestTimerControlsProps) {
  ...
  return (
    <View className="w-full max-w-sm flex-row items-center justify-center gap-4">
      <Pressable
        className="active:bg-bg-card-elevated bg-bg-overlay/80 h-12 items-center justify-center rounded-xl border active:scale-95"
        style={{ minWidth: theme.size['18'], borderColor: theme.colors.overlay.white5 }}
        onPress={onMinus5s}
      >
        <Text className="text-text-primary text-sm font-bold">{t('restTimer.minus5s')}</Text>
      </Pressable>
      ...
      <Pressable
        ...
        onPress={onPlus5s}
      >
        <Text className="text-text-primary text-sm font-bold">{t('restTimer.plus5s')}</Text>
      </Pressable>
    </View>
  );
}
