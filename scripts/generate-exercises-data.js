 ...
 *                      It is a display-order and locale-join key ONLY — database
 *                      primary keys are `fx-<__freeExerciseDbId>`, so re-running
 *                      this generator after the source repo gains an exercise
 *                      renumbers rows without touching any stored id.
...
// free-exercise-db name -> the legacy catalogue exercise it is the same
// movement as. These inherit `loadMultiplier` verbatim, so the new catalogue
// stays anchored to the scale already established in `legacyExercisesData.json`.
