...
| TODO                                      | Location                          | Plan                                                                                                                                                                                                                                                                                                                                    | Effort | Status |
| ----------------------------------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------ |
| Implement onboarding "Gate"               | `app/onboarding/onboarding.tsx`   | Add a new step or branch in the onboarding pager: "Do you experience a menstrual cycle?" (Yes/No). If "Yes," navigate to the Cycle Setup flow.                                                                                                                                                                                          | S      | —      |
| Implement Cycle Onboarding Flow           | `app/onboarding/`                 | Create a 4-step setup: 1. `cycle-anchor.tsx` (Calendar picker for last period start); 2. `cycle-birth-control.tsx` (Selectors for Pill, IUD, None); 3. `cycle-length.tsx` (Sliders for avg cycle length and period duration); 4. `cycle-goal.tsx` (Radio buttons for "Optimize Performance", "Manage Symptoms", "Predict Energy Lows"). | M      | —      |
| Update Data Models (Sync Goal & Metrics)  | `database/`                       | 1. Update `MenstrualCycle.ts` & schema to include `sync_goal` field. 2. Add `period_flow`, `period_symptoms`, and `basal_body_temp` to `UserMetricType` in `UserMetric.ts`.                                                                                                                                                             | S      | —      |
| Develop Menstrual Logic Service           | `database/services/`              | Create `MenstrualService.ts` with logic for: `calculateCurrentPhase()` (Menstrual, Follicular, Ovulation, Luteal); `getEnergyLevel()` (Peak, High, Moderate, Low); and `getIntensityMultiplier(goal)` (e.g., 0.8 during Menstrual for deloading, 1.1 during Ovulation for PRs).                                                         | M      | —      |
| Implement "Focus" Dashboard UI            | `app/focus.tsx`                   | Build the high-level dashboard from mockups: `CyclePhaseWheel` (4-phase visualizer), `EnergyLevelBadge` (e.g., "Peak"), `WeeklyTrainingPlan` (labeling days as "High Intensity" or "Active Recovery"), and `PhysiologicalInsightsCard` (Estrogen/Metabolism trends).                                                                    | L      | —      |
| Implement navigation for Hormonal Syncing | `components/NavigationMenu.tsx`   | Update bottom navigation to match mockups when cycle tracking is active: [Focus, Workouts, Cycle, Stats, Profile]. Ensure the "Focus" tab is the primary entry point for hormonal insights.                                                                                                                                             | S      | —      |
| Implement daily symptom & flow logging    | `components/modals/`              | Create `CycleLogModal.tsx`: interactive logging for "Flow Intensity" and "Symptoms" (Cramps, Mood, Energy). Persist via `UserMetricService` with encrypted notes in `user_metrics_notes`.                                                                                                                                               | M      | —      |
| Adaptive Workout session integration      | `app/workout/workout-session.tsx` | On session start, check `MenstrualService.getIntensityMultiplier()`. Show a "Physiological Insight" card suggesting adjustments (e.g., "Hormonal Peak: Great day for a PR" or "Low Energy: Consider 10% weight reduction").                                                                                                             | M      | —      |
| Update UserMetrics for BBT                | `database/services/`              | Update `UserMetricService.ts` to support `basal_body_temp`. Integration with wearables (via Health Connect) should automatically populate this metric to help verify ovulation timing.                                                                                                                                                  | S      | —      |
