...
`npm run generate-exercises-data -- [path-to-free-exercise-db]` regenerates the structural and English files from the upstream checkout. Its entry point delegates structural mapping, multiplier policy, and the stable popular-exercise selection to focused modules under `scripts/`. Run `npm run generate-exercise-locales` immediately afterwards to regenerate every translated copy before committing a catalogue update; `npm run generate-lang` then discovers those files and rebuilds `EXERCISES_JSON` in `lang/lang.ts`.

free-exercise-db has no popularity field and its Musclog import is alphabetically ordered. `scripts/exercise-popularity-policy.js` therefore freezes exactly 100 stable upstream slugs selected from usage-based exercise rankings; `data/exercisesData.json` adds `isPopular: true` to those rows and omits the property from every other row. The policy records the source snapshot and one-to-one mapping rules. Edit it and regenerate instead of hand-editing the JSON, adding `false` values, deriving the flag from `exerciseIndex`, or applying one generic movement's rank to several local variants. This flag is structural catalogue metadata only; the WatermelonDB exercise projection does not currently persist it.
