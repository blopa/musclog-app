...
- Game Boy optical export: Settings → Share Data sends the entire cartridge database as an optimized, screen-filling fountain-coded QR stream. The Android, iOS, or web app verifies it, expands its compact versioned JSON schema, previews it as a database restore, and replaces the receiving database only after confirmation; pressing B asks whether to continue or stop, and the Game Boy remains sender-only with no Game Boy-to-Game Boy transfer
