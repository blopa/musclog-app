...
- Game Boy optical export: Settings → Share Data sends the entire cartridge database as an endless fountain-coded QR stream. The Android, iOS, or web app verifies it, expands its compact versioned JSON schema, previews it as a database restore, and replaces the receiving database only after confirmation; the Game Boy is sender-only and does not support Game Boy-to-Game Boy transfer
