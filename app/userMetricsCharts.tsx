...
const UserMetricsCharts = ({ navigation }: { navigation: NavigationProp<any> }) => {
    ...
    const [ffmi, setFFMI] = useState<{ ffmi: string; normalizedFFMI: string } | undefined>(
        undefined
    );
    ...
    const calculateWeeklyNutritionAverages = useCallback(
        (data: NutritionStackedBarChartDataType[]) => {
            ...
            data.forEach((dataPoint, index) => {
                ...
                if (currentWeek.length === 7 || index === data.length - 1) {
                    const totalCarbs =
                        currentWeek.reduce((sum, item) => sum + (item.y[0] || 0), 0) /
                        CALORIES_IN_CARBS;
                    const totalFats =
                        currentWeek.reduce((sum, item) => sum + (item.y[1] || 0), 0) /
                        CALORIES_IN_FAT;
                    const totalProteins =
                        currentWeek.reduce((sum, item) => sum + (item.y[2] || 0), 0) /
                        CALORIES_IN_PROTEIN;
                    const totalFibers =
                        currentWeek.reduce((sum, item) => sum + (item.y[3] || 0), 0) /
                        CALORIES_IN_FIBER;
                    const totalCalories = currentWeek.reduce(
                        (sum, item) => sum + Number(item.totalCalories),
                        0
                    );
                    ...
                    const averageProteins =
                        totalProteins / (currentWeek.length - daysWithNoProteins) || 0;
                    const averageFibers =
                        totalFibers / (currentWeek.length - daysWithNoFibers) || 0;
    ...
    const prepareNutritionData = useCallback(
        (data: UserNutritionDecryptedReturnType[]) => {
            ...
            return Object.entries(aggregatedData).map(([date, nutritionData], index) => {
                ...
                if (calorieDifference !== 0) {
                    ...
                    const weightSum =
                        CALORIES_IN_CARBS * nutritionData.carbohydrate +
                    ...
                    if (weightSum > 0) {
                        yData[0] +=
                            (calorieDifference * (CALORIES_IN_CARBS * nutritionData.carbohydrate)) /
                            weightSum;
                        yData[1] +=
                            (calorieDifference * (CALORIES_IN_FAT * nutritionData.fat)) / weightSum;
                        yData[2] +=
                            (calorieDifference * (CALORIES_IN_PROTEIN * nutritionData.protein)) /
                            weightSum;
                        yData[3] +=
                            (calorieDifference * (CALORIES_IN_FIBER * nutritionData.fiber)) /
                            weightSum;
                    }
                }
            ...
            if (isWeeklyAverages) {
                ...
                aggregatedData.forEach((data, index) => {
                    ...
                    if (currentDate <= endDate) {
                        ...
                        while (currentDate > endDate) {
                            if (
                                currentWeekData.nutritionData.length > 0 ||
                                currentWeekData.weightData.length > 0
                            ) {
                                const avgNutritionData = currentWeekData.nutritionData.reduce(
                                    (acc, item) => {
                                ...
                                avgNutritionData.y = avgNutritionData.y.map(
                                    (val) => val / currentWeekData.nutritionData.length
                                );

                                const avgWeightData =
                                    currentWeekData.weightData.reduce(
                                        (acc, item) => acc + item.y,
                                        0
                                    ) / currentWeekData.weightData.length;
                    ...
                    if (
                        ...
                        const avgWeightData =
                            currentWeekData.weightData.reduce((acc, item) => acc + item.y, 0) /
                            currentWeekData.weightData.length;
    ...
    const loadUserMetricsAndNutrition = useCallback(async () => {
        ...
        try {
            ...
            if (startDate && endDate) {
                loadedUserMetrics = await getUserMetricsBetweenDates(
                    startDate.toISOString(),
                    endDate.toISOString()
                );
                loadedUserNutrition = await getUserNutritionBetweenDates(
                    startDate.toISOString(),
                    endDate.toISOString()
                );
                recentWorkouts = await getRecentWorkoutsBetweenDates(
                    startDate.toISOString(),
                    endDate.toISOString()
                );
                userMeasurements = await getUserMeasurementsBetweenDates(
                    startDate.toISOString(),
                    endDate.toISOString()
                );

                loadedUserMetrics = loadedUserMetrics.sort(
                    (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
                );

                loadedUserNutrition = loadedUserNutrition.sort(
                    (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
                );

                recentWorkouts = recentWorkouts.sort(
                    (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
                );

                userMeasurements = userMeasurements.sort(
                    (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
                );
            } else {
            ...
            for (const [index, workout] of recentWorkouts.entries()) {
                const exerciseData = JSON.parse(workout?.exerciseData || '[]') as {
                    exerciseId: number;
                    sets: SetReturnType[];
                }[];

                const workoutVolume =
                    parseFloat(workout.workoutVolume || '0') ||
                    (await calculateWorkoutVolume(exerciseData)) ||
                    0;
            ...
            const recentWorkoutsDataToShow = showWeeklyAverages
                ? calculatePastWorkoutsWeeklyAverages(recentWorkoutsChartData)
                : recentWorkoutsChartData;

            const filteredRecentWorkoutsData = recentWorkoutsDataToShow.filter(
                (workout) => workout.y
            );
            ...
            const nutritionDataWithoutTotals = showWeeklyAverages
                ? calculateWeeklyNutritionAverages(filteredNutritionData)
            ...
            const totalCaloriesLabel = nutritionDataWithoutTotals.map((item) =>
                t('value_kcal', { value: item.totalCalories })
            );
            ...
            // User metrics
            const sortedUserMetrics = loadedUserMetrics.sort(
                (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
            );

            const filteredWeightMetrics = sortedUserMetrics.filter(
                (metric) => metric.date && metric.weight
            );
            // .slice(-parseInt(dateRange))
            const filteredFatPercentageMetrics = sortedUserMetrics.filter(
                (metric) => metric.date && metric.fatPercentage
            );
            // .slice(-parseInt(dateRange))
            ...
            const weightDataToShow = showWeeklyAverages
                ? calculateWeeklyAverages(sortedUserMetrics, 'weight')
                : weightValues;
            const fatPercentageDataToShow = showWeeklyAverages
                ? calculateWeeklyAverages(sortedUserMetrics, 'fatPercentage')
                : fatPercentageValues;

            if (sortedUserMetrics.length > 7) {
                const userMetricsByWeek = aggregateDataByWeek(sortedUserMetrics);

                const userMetricsByWeekWithAverages = userMetricsByWeek.map(
                    ({ data, weekEnd, weekStart }, index) => {
                        const validWeightPoints = data.filter((item) => item.weight);
                        const summedWeight = validWeightPoints.reduce(
                            (sum, item) => sum + (item.weight || 0),
                            0
                        );

                        const validFatPercentagePoints = data.filter((item) => item.fatPercentage);
                        const summedFatPercentage = validFatPercentagePoints.reduce(
                            (sum, item) => sum + (item.fatPercentage || 0),
                            0
                        );

                        return {
                            averageFatPercentage: summedFatPercentage
                                ? summedFatPercentage / validFatPercentagePoints.length
                                : undefined,
                            averageWeight: summedWeight
                                ? summedWeight / validWeightPoints.length
                                : undefined,
                            data,
                            weekEnd,
                            weekStart,
                ...
                const weightTrend = userMetricsByWeekWithAverages
                    .map((week, index) => {
                        if (
                            index === 0 ||
                            !week.averageWeight ||
                            !userMetricsByWeekWithAverages[index - 1].averageWeight
                        ) {
                            return null;
                        }
                ...
                const fatPercentageTrend = userMetricsByWeekWithAverages
                    .map((week, index) => {
                        if (
                            index === 0 ||
                            !week.averageFatPercentage ||
                            !userMetricsByWeekWithAverages[index - 1].averageFatPercentage
                        ) {
                            return null;
                        }
                ...
                const averageWeightDifference =
                    weightTrend.reduce((sum, { average: val }) => sum + val, 0) /
                    weightTrend.length;
                const averageFatPercentageDifference =
                    fatPercentageTrend.reduce((sum, { average: val }) => sum + val, 0) /
                    fatPercentageTrend.length;

                const averageWeight =
                    weightTrend.reduce((sum, { averageWeight: val }) => sum + val, 0) /
                    weightTrend.length;
                const averageFatPercentage =
                    fatPercentageTrend.reduce(
                        (sum, { averageFatPercentage: val }) => sum + val,
                        0
                    ) / fatPercentageTrend.length;
            ...
            const weightLabelsToShow = showWeeklyAverages
                ? (weightDataToShow as AverageWeekDataType[]).map(
                      (data) => `${data.startDate}-${data.endDate}`
                  )
                : weightDateLabels;

            const fatPercentageLabelsToShow = showWeeklyAverages
                ? (fatPercentageDataToShow as AverageWeekDataType[]).map(
                      (data) => `${data.startDate}-${data.endDate}`
                  )
            ...
            const { totalDays, weeklyAverages: userMetricsNutritionWeeklyAverages } =
                calculateUserMetricsNutritionWeeklyAverages(aggregatedUserMetricsNutrition);

            // Calculate calories with reduce
            const totalCalories = userMetricsNutritionWeeklyAverages.reduce(
                (sum, item) => sum + item.averageCalories,
                0
            );
            ...
            const initialFatPercentage =
                userMetricsNutritionWeeklyAverages?.at(0)?.averageFatPercentage || 0;
            const finalFatPercentage =
                userMetricsNutritionWeeklyAverages?.at(-1)?.averageFatPercentage || 0;

            const useFatPercentageTDEESetting = await getSettingByType(
                USE_FAT_PERCENTAGE_TDEE_TYPE
            );
            ...
            if (user?.metrics?.height) {
                ...
                const aggregatedMetrics = aggregateMetricsByDate(
                    sortedUserMetrics
    ...
    useFocusEffect(
        ...
        }, [showWeeklyAverages, shouldReloadData, dateRange])
    );
    ...
    const yAxisFood = useMemo(() => {
        return {
            axisMaximum: Math.round(
                Math.max(...foodChartData.flatMap((d) => d.y.reduce((acc, val) => acc + val, 0))) *
                    1.2
            ),
            axisMinimum: 0,
        };
    ...
    const fabActions = useMemo(() => {
        const actions = [
            {
                icon: () => (
                    <FontAwesome5 color={colors.primary} name="database" size={FAB_ICON_SIZE} />
                ),
            ...
            {
                icon: () => (
                    <FontAwesome5 color={colors.primary} name="sync" size={FAB_ICON_SIZE} />
                ),
                    ...
                    >
                        ...
                        {ffmiData.length > 0 ? (
                            <LineChart
                                ...
                                yAxis={{
                                    axisMaximum: Math.round(
                                        Math.max(...ffmiData.map((d) => d.y)) * 1.05
                                    ),
                                    axisMinimum: Math.round(
                                        Math.min(...ffmiData.map((d) => d.y)) * 0.95
                                    ),
                                }}
                                yAxisLabel={t('ffmi')}
                            />
                        ...
                        {recentWorkoutsData.length > 0 ? (
                            <LineChart
                                ...
                                yAxis={{
                                    axisMaximum: Math.round(
                                        Math.max(...recentWorkoutsData.map((d) => d.y)) * 1.2
                                    ),
                                    axisMinimum: Math.round(
                                        Math.min(...recentWorkoutsData.map((d) => d.y)) * 0.5
                                    ),
                                }}
                        ...
                        {Object.keys(measurementsData).map((measurementKey) => (
                            <LineChart
                                ...
                                yAxis={{
                                    axisMaximum: Math.round(
                                        Math.max(
                                            ...measurementsData[measurementKey].map((d) => d.y)
                                        ) * 1.2
                                    ),
                                    axisMinimum: Math.round(
                                        Math.min(
