...
import { NutritionCheckinService, CheckinMetrics } from '../../database/services/NutritionCheckinService';
...
export default function CheckinReviewScreen() {
  ...
  const displayActualWeight = kgToDisplay(avgWeight, weightUnit);
  const displayTargetWeight = kgToDisplay(checkin.targetWeight, weightUnit);
  const displayTrend = kgToDisplay(Math.abs(trend), weightUnit);
  const trendColor = trend <= 0 ? theme.colors.status.emerald : theme.colors.status.warning;
  const weightUnitKey = weightUnit === 'metric' ? 'common.weightFormatKg' : 'common.weightFormatLbs';
    ...
    <MasterLayout showNavigationMenu={false}>
      <View className="flex-1 bg-[#0F172A]">
        ...
        <ScrollView className="flex-1" contentContainerStyle={{ padding: 24, paddingBottom: 160 }}>
          ...
          {!hasEnoughData && (
            <View className="mb-8 flex-row items-center gap-3 rounded-2xl bg-[#fbbf24]20 p-4 border border-[#fbbf24]40">
              <AlertCircle size={20} color="#fbbf24" />
              <Text className="flex-1 text-sm font-medium text-[#fbbf24]">
            ...
            <View className="flex-row justify-between">
              <View>
                ...
                <View className="mt-1 flex-row items-baseline">
                  ...
                  <Text className="ml-1 text-base font-bold text-gray-400">{t(weightUnitKey, { value: '' })}</Text>
                  <View
              ...
              <View className="items-end">
                ...
                <Text className="mt-1 text-xl font-bold text-white">
                  ...
                  <Text className="text-sm font-medium text-gray-400"> {t(weightUnitKey, { value: '' })}</Text>
                </Text>
              </View>
            </View>
