...
describe('calculateEmpiricalTDEEWindow', () => {
  it('returns fallback window when weight data is insufficient', () => {
    ...
    const startDate = START_DATE - 10 * MS_PER_SOLAR_DAY;
    const endDate = START_DATE + 10 * MS_PER_SOLAR_DAY;
  ...
  it('uses first and last weight measurements as fallback if at least 2 exist but no matched BF', () => {
    const weightPoints: MetricPoint[] = [
      ...
      { date: START_DATE + 10 * MS_PER_SOLAR_DAY, value: 95 },
    ];
    const fatPoints: MetricPoint[] = [];
    ...
    expect(result.empiricalEnd).toBe(START_DATE + 10 * MS_PER_SOLAR_DAY);
  ...
  it('picks exactly 2 matched data points', () => {
    const weightPoints: MetricPoint[] = [
      { date: START_DATE, value: 100 },
      { date: START_DATE + 5 * MS_PER_SOLAR_DAY, value: 98 },
      { date: START_DATE + 10 * MS_PER_SOLAR_DAY, value: 95 },
    ];
    const fatPoints: MetricPoint[] = [
      { date: START_DATE, value: 20 },
      { date: START_DATE + 10 * MS_PER_SOLAR_DAY, value: 18 },
    ];
    ...
    expect(result.empiricalDays).toBe(
      Math.max(0, Math.ceil((dayKey(10) - dayKey(0)) / MS_PER_SOLAR_DAY))
    );
  });
  ...
  it('correctly identifies Matched Data Window when measurements are sparse', () => {
    const weightPoints: MetricPoint[] = [
      { date: START_DATE, value: 100 },
      { date: START_DATE + 1 * MS_PER_SOLAR_DAY, value: 99 },
      { date: START_DATE + 2 * MS_PER_SOLAR_DAY, value: 98 },
      { date: START_DATE + 3 * MS_PER_SOLAR_DAY, value: 97 },
    ];
    const fatPoints: MetricPoint[] = [
      { date: START_DATE, value: 20 },
      { date: START_DATE + 1 * MS_PER_SOLAR_DAY, value: 19.5 },
      { date: START_DATE + 3 * MS_PER_SOLAR_DAY, value: 19 },
      { date: START_DATE + 4 * MS_PER_SOLAR_DAY, value: 18.5 },
    ];
    ...
    expect(result.empiricalDays).toBe(
      Math.max(0, Math.ceil((dayKey(3) - dayKey(0)) / MS_PER_SOLAR_DAY))
    );
  });
  ...
  it('does NOT use weekly averaging for exactly 14 points', () => {
    ...
    const fatPoints: MetricPoint[] = [];
    for (let i = 0; i < 14; i++) {
      weightPoints.push({ date: START_DATE + i * MS_PER_SOLAR_DAY, value: 100 - i });
      fatPoints.push({ date: START_DATE + i * MS_PER_SOLAR_DAY, value: 20 - i * 0.1 });
    }
    ...
    expect(result.empiricalDays).toBe(
      Math.max(0, Math.ceil((dayKey(13) - dayKey(0)) / MS_PER_SOLAR_DAY))
    );
  });
  ...
  it('handles mismatched sparse data with averaging', () => {
    ...
    const weightPoints: MetricPoint[] = matchedDays.map((d) => ({
      date: START_DATE + d * MS_PER_SOLAR_DAY,
      value: 100 - d,
    }));
