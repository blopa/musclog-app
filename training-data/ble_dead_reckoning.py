...
dead-reckoning and foot-mounted IMU navigation literature):
  ...
  1. Body → world rotation        scipy.spatial.transform.Rotation
  2. Subtract nominal gravity     numpy
  3. ZUPT detection               scipy.ndimage.uniform_filter1d
  4. Bias estimate + subtract     numpy.interp
  5. Velocity by integration      scipy.integrate.cumulative_trapezoid
  6. ZUPT velocity ramp fix       numpy (vectorised per segment)
  7. Position by integration      scipy.integrate.cumulative_trapezoid
...
def compute_position(
    ...
    *,
    ...
    See module docstring for the algorithm and references.

    Returns a dict with ``timestamps_ms``, ``position_m`` (N,3), ``velocity_ms``,
    ``linear_acc_ms2``, ``accel_bias_g``, ``stationary``, ``sr_hz``.
    """
    ...
    # 1. Body → world. WT901 uses ZYX intrinsic; scipy 'ZYX' expects [yaw, pitch, roll].
    rotations     = Rotation.from_euler("ZYX", angle_deg[:, [2, 1, 0]], degrees=True)
    accel_world_g = rotations.apply(accel_g)

    # 2. Subtract nominal gravity (will leave a residual bias — fixed in step 4).
    linear_acc_g_biased = accel_world_g - np.array([0.0, 0.0, 1.0])

    # 3. ZUPT detection: rolling std of |accel| and mean of |gyro|.
    ...
    accel_std     = np.sqrt(np.maximum(0.0, accel_mean_sq - accel_mean * accel_mean))
    ...
    # 5. Velocity = ∫ linear_acc dt  (trapezoidal, vectorised).
    velocity = cumulative_trapezoid(linear_acc_ms2, t_s, axis=0, initial=0.0)
    velocity[stationary] = 0.0  # classical ZUPT

    # 6. Linear ramp correction across each motion segment: residual velocity at
    #    the end of motion must be 0 (entering stationary); spread the leftover
    #    drift linearly back across the segment.
